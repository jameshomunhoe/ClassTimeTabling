 ...
 // for(venue = 0 ; venue < totalVenue ; venue++){
  // for(time = 0 ; time < MAX_TIME_SLOT ; time++){
    // if(classIsNull(newClass[venue][dayToCheck][time]) == 0){
      // if(newClass[venue][dayToCheck][time].typeOfClass == Lecture){
        // a = courseGetNumberOfCombinedGroups(newClass[venue][dayToCheck][time].course);
        // for(i = 0 ; i < a ; i++){
          // courseGetCombinedGroups(newClass[venue][dayToCheck][time].course,i ,)
        // }
      // }
      // else
    // newClass[venue][dayToCheck][time]
    // getIndexInList
    // courseGetCombinedGroups
    // }
  // }
 // }
